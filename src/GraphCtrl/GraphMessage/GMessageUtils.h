...
#define CGRAPH_CREATE_MESSAGE_TOPIC(Type, topic, size)                              \
    GMessageManagerSingletonPtr->createTopic<Type>(topic, size);                    \

/* 删除一个 topic */
#define CGRAPH_REMOVE_MESSAGE_TOPIC(topic)                                          \
    GMessageManagerSingletonPtr->removeTopic(topic);                                \

/* 发送一个 message param */
#define CGRAPH_PUB_MPARAM(Type, topic, value)                                       \
    GMessageManagerSingletonPtr->pubTopicValue<Type>(topic, value);                 \

/* 接收一个 message param */
#define CGRAPH_SUB_MPARAM(Type, topic, value)                                       \
    GMessageManagerSingletonPtr->subTopicValue<Type>(topic, value);                 \

/* 清空所有的topic */
#define CGRAPH_CLEAR_MESSAGES()                                                     \
    GMessageManagerSingletonPtr->clear();                                           \
